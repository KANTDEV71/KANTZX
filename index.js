...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:MEU CRIADOR🙈\n' 
            + 'ORG: kant;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=5511942347051:+55 11 94234-7051\n' 
            + 'END:VCARD' 
blocked = []   
fake = 'kml'
prefix = '#'
...
/******** OWNER NUMBER**********/
const ownerNumber = ["5511942347051@s.whatsapp.net","5511942347051@s.whatsapp.net"] 
/************************************/
...
case 'spam':
    			if (!isRegistered) return reply(ind.noregis())
		    		if (isLimit(sender)) return reply(ind.limitend(pusname))
			     	await limitAdd(sender)	    				    					     	
                    if (args.length < 1) return reply(`Use assim:  exemplo : ${prefix}spam 5511942347051 corno`)
...
break
		    ...
		    ow = "5511942347051@s.whatsapp.net"
		    cuy = "5511942347051@s.whatsapp.net"
                    menusu = `
❏ *GRUPO MENU*
│
└◪› ${prefix}welcome [1/0]
...
break
    ...
    case 'transferi':
                ...
                addKoinUser('5511942347051@s.whatsapp.net', fee)
...
.on('end', function () {
console.log('Finish')
exec(`webpmux -set exif ${addMetadata('AKAME-BOT', 'kant')} ${ran} -o ${ran}`, async (error) => {
		  ...
		  if (!isBotGroupAdmins) return reply(dpa.badmin)
				...
				case 'tm': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ AKAME/KANT ʙᴏᴛ ❯\n\n${body.slice(4)}`})
						}
						reply('[TM] de foto feita com sucesso ✔')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `❮ AKAMI/KANT ʙᴏᴛ ❯\n\n${body.slice(4)}`)
						}
